...
!!! note
    ...
    Важливо знати, що, отримавши двічі поспіль подію `emit` зі статусом Invoic'у `pay`, це свідчить про те, що трейдер змінив ціну. Якщо отриманий статус Invoic'у `waitPhoto`, і при цьому поле "approve_photo" не є порожнім, це вказує на те, що ваш клієнт вже завантажив фото і чекає на рішення адміністратора.
...
1. Під'єднання до [WebSocket'у](#socketio-emit), відправка **emit'у** при приєднанні, та отримання **emit'y** в відповідь з IP того хто перший раз приєднався.
